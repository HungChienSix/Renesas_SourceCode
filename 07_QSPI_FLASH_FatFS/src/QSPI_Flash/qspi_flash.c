...
uint32_t QSPI_Flash_ReadID(void)
{
    ...
    R_QSPI_DirectWrite(&g_qspi0_ctrl, &data[0], 1, true);     //false: close the spi  true: go go go
...
uint32_t QSPI_Flash_ReadDeviceID(void)
{
    ...
    data[2] = 0xff;
    data[3] = 0xff;
    R_QSPI_DirectWrite(&g_qspi0_ctrl, &data[0], 4, true);     //false: close the spi  true: go go go
    R_QSPI_DirectRead(&g_qspi0_ctrl, &data[0], 3);
...
void QSPI_Flash_PowerDown(void)
{
    unsigned char data[6] = {};
    data[0] = PowerDown;
    R_QSPI_DirectWrite(&g_qspi0_ctrl, &data[0], 1, false);
}
...
{
    R_QSPI_Write(&g_qspi0_ctrl, pBuffer, WriteAddr, NumByteToWrite);
    QSPI_Flash_WaitForWriteEnd();
}
 ...
 * @retval 无
 */
fsp_err_t QSPI_Flash_BufferWrite(uint8_t *pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
    ...
    if (Addr == 0)
    {
        ...
        else /* NumByteToWrite > SPI_FLASH_PageSize */
        {
            ...
            while (NumOfPage--)
            {
                R_QSPI_Write(&g_qspi0_ctrl, pBuffer, WriteAddr, SPI_FLASH_PageSize);
                QSPI_Flash_WaitForWriteEnd();
            ...
            /*若有多余的不满一页的数据，把它写完*/
            R_QSPI_Write(&g_qspi0_ctrl, pBuffer, WriteAddr, NumOfSingle);
            QSPI_Flash_WaitForWriteEnd();
    ...
    else
    {
        ...
        if (NumOfPage == 0)
        {
            ...
            if (NumOfSingle > count)
            {
                ...
                /*先写满当前页*/
                R_QSPI_Write(&g_qspi0_ctrl, pBuffer, WriteAddr, count);
                QSPI_Flash_WaitForWriteEnd();
                ...
                pBuffer += count;
                /*再写剩余的数据*/
                R_QSPI_Write(&g_qspi0_ctrl, pBuffer, WriteAddr, temp);
                QSPI_Flash_WaitForWriteEnd();
...
* @retval 无
*/
fsp_err_t QSPI_Flash_BufferRead(uint8_t *pBuffer, uint32_t ReadAddr, uint16_t NumByteToRead)
{
    return R_QSPI_Read(&g_qspi0_ctrl, pBuffer, ReadAddr, NumByteToRead);
}
