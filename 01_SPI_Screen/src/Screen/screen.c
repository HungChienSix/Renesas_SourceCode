								...
								ST7735_Pixel_t Pixel_Set, SCREEN_Mode_t type)
{
	...
	static int16_t arc_points[SCREEN_MAX_ARC_POINTS][2]; /* 增加数组大小，支持更大的半径 */
	...
	if (r != last_r) {
		...
		while(x <= y) {
			/* 边界检查：防止数组越界 */ 
			if (point_count >= SCREEN_MAX_ARC_POINTS) {
				break;
			}
									...
									ST7735_Pixel_t Pixel_Set, SCREEN_Mode_t type)
{
	...
	/* 计算表大小（首次调用时扫描，后续缓存复用） */
	static const struFont_UTF_data_t *cached_table = NULL;
	static int table_size = 0;
	if (cached_table != table) {
		table_size = 0;
		while (!(table[table_size].UTF8_code[0] == 0 &&
