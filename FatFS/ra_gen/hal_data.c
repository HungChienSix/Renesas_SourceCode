...
{ .clk_div =
{
/* Actual calculated bitrate: 8333333. */.cks = 0,
  .brr = 2, .mddr = 0, } };
...
/* Actual period: 0.001 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x186a0,
  .duty_cycle_counts = 0xc350, .source_div = (timer_source_div_t) 0, .channel = 0, .p_callback = TIM_Clock_Callback,
...
{ .mode = TIMER_MODE_PERIODIC,
/* Actual period: 7e-7 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x46,
  .duty_cycle_counts = 0x23, .source_div = (timer_source_div_t) 0, .channel = 2, .p_callback = NULL,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
