/**************************************************************************//**
 * @file     FlashPrg.c
 * @brief    Flash Programming Functions adapted for RA2L2
 * @version  V1.0.0
 * @date     22 Apr 2024
 ******************************************************************************/
/*
 ...
 *         : 22.04.2024 1.0.0    First Release
 *********************************************************************************************************************/
...
#define MCU_SOPCCR_ADDR									(MCU_SYSTEM_BASE + 0x0AA)
		...
		#define SOPCCR_SOPCMTSF_TRANS				(0x01)
...
#define MCU_FRESETR_ADDR               (MCU_FLASH_BASE + 0x124)
    #define FRESETR_FRESET_MASK        (0x0)
...
#define CODEF_SIZE											(0x00040000) // Should re-define this value base on user's marco
#define DATAF_SIZE											(0x00002000) // Should re-define this value base on user's marco
...
static BYTE changeModeAndClock(DWORD *clk)
{
	...
	if ((userRegisterValue[OPCCR] & OPCCR_OPCM) == OPCCR_OPCM_HIGH){
		...
		if ((userRegisterValue[OFS1] & OFS1_HOCOFRQ1_MASK) != OFS1_HOCOFRQ1_24MHZ && (userRegisterValue[OFS1] & OFS1_HOCOFRQ1_MASK) != OFS1_HOCOFRQ1_32MHZ && (userRegisterValue[OFS1] & OFS1_HOCOFRQ1_MASK) != OFS1_HOCOFRQ1_48MHZ && (userRegisterValue[OFS1] & OFS1_HOCOFRQ1_MASK) != OFS1_HOCOFRQ1_64MHZ)
		{
			// Change clock source to MOCO
			if (userRegisterValue[SCKSCR] != SCKSCR_CKSEL_MOCO)
			{
		...
		{ // Otherwise, use HOCO
			...
			if (userRegisterValue[SCKSCR] != SCKSCR_CKSEL_HOCO)
			{
...
static void Wait_us(DWORD us)
{
	...
	if (dwTotalNsPerCycle < (1000000000 / MAX_OPERATING_FREQUENCY)) // T = dwTotalNsPerCycle = 10^9 / f < 21
	{
