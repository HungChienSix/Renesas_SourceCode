/**************************************************************************//**
 * @file     FlashPrg.c
 * @brief    Flash Programming Functions adapted for RA2T1
 * @version  V1.0.0
 * @date     20 June 2024
 ******************************************************************************/
/*
 ...
 *         : 20.06.2024 1.0.0    First Release
 *********************************************************************************************************************/
...
#define MCU_SCKDIVCR_ADDR               (MCU_SYSTEM_BASE+0x020)
    #define SCKDIVCR_ICLK_1DEV		   		(0x00000000)
    #define SCKDIVCR_ICLK_2DEV     			(0x01000101)
		#define SCKDIVCR_ICLK_4DEV     			(0x02000202)
...
/*
   Other parameters
*/
#define MAX_OPERATING_FREQUENCY					(64000000)
#define RES_ERROR												(1)
#define RES_OK													(0)
#define FLASH_WRITEABLE_MIN_CLOCK_HZ		(1000000)
#define FLASH_WRITEABLE_MAX_CLOCK_HZ_HIGH		(64000000)
...
#define MCU_CODEF_SIZE									(0x00010000) // Should re-define this value base on user's marco
#define MCU_DATAF_SIZE									(0x00000800) // Should re-define this value base on user's marco
...
#define MCU_OSIS4_ADDR									(CONFIG_START + 0x30)
    ...
    #define MCU_CODEF_SIZE							(0x00010000)
...
int Init (unsigned long adr, unsigned long clk, unsigned long fnc)
{
  ...
  byteVal = checkClockAndMode(&clk); // Check if flash rewritable clock and power control mode 
	dwICLKValue = clk; // Save current frequency for dwICLKValue
  ...
  if(byteVal)
  {
	...
	{
		M8(MCU_FISR_ADDR) = (M8(MCU_FISR_ADDR) | ((byteClk >> 1) + 15));
	}
...
int ProgramPage (unsigned long adr, unsigned long sz, unsigned char *buf)
{
	...
	} else if (adr < MCU_DATAF_BASEADDR)
	{
		...
		// Update parameter for Configuration Area
...
static DWORD checkClockAndMode(DWORD *clk)
{
  ...
  switch (wClockUserVal[SCKSCR])
  {
    ...
    case SCKSCR_CKSEL_HOCO:
      ...
      break;
    case SCKSCR_CKSEL_LOCO: // Failed because LOCO frequency is below 1MHz
				dwordClk = 32768;
				break; 	
    default:
        return (RES_ERROR); // Failed
		...
		dwordSckdivcr = dwordSckdivcr >> 24;
    dwordSckdivcrVal = 1 << dwordSckdivcr; // ICLK division value
    *clk = dwordClk / dwordSckdivcrVal; // Calculate ICLK frequency
  ...
  } else if ((wClockUserVal[OPCCR] & OPCCR_OPCM) == OPCCR_OPCM_LOW)
  {
...
static DWORD changeClockAndMode(DWORD *clk)
{
  ...
  if ((wClockUserVal[SOPCCR] & SOPCCR_SOPCM) != SOPCCR_SOPCM_OTH)
  {
	...
	if ((wClockUserVal[OPCCR] & OPCCR_OPCM) == OPCCR_OPCM_HIGH){
		...
		if ((wClockUserVal[OFS1] & OFS1_HOCOFRQ1_MASK) != OFS1_HOCOFRQ1_24MHZ && (wClockUserVal[OFS1] & OFS1_HOCOFRQ1_MASK) != OFS1_HOCOFRQ1_32MHZ && (wClockUserVal[OFS1] & OFS1_HOCOFRQ1_MASK) != OFS1_HOCOFRQ1_48MHZ && (wClockUserVal[OFS1] & OFS1_HOCOFRQ1_MASK) != OFS1_HOCOFRQ1_64MHZ)
		{
      /** If the clock source is not MOCO, set the clock source to MOCO */
      ...
      *clk = 8000000; // FCLK frequency is 8MHz
      ...
      {		
          ...
          M16(MCU_SCKSCR_ADDR) = SCKSCR_CKSEL_HOCO;// Set the clock source to HOCO
			...
			M32(MCU_SCKDIVCR_ADDR) = SCKDIVCR_ICLK_1DEV;
	...
	} else
	{
      ...
      if (wClockUserVal[SCKSCR] != SCKSCR_CKSEL_MOCO)
      {
          ...
          M16(MCU_SCKSCR_ADDR) = SCKSCR_CKSEL_MOCO;// Set the clock source to MOCO
			...
			if ((wClockUserVal[OPCCR] & OPCCR_OPCM) == OPCCR_OPCM_MID) // Middle-speed mode
			{
				// Change clock division to 2
				M32(MCU_SCKDIVCR_ADDR) = SCKDIVCR_ICLK_2DEV;
				*clk = 4000000;
			} else // Low-speed mode
			{
				// Change clock division to 4
				M32(MCU_SCKDIVCR_ADDR) = SCKDIVCR_ICLK_4DEV;
				*clk = 2000000;
			}
...
static BYTE enterPEMode(DWORD adr, DWORD *adrRet)
{
	...
	} else
	{
		...
		Wait_us(1); // Wait for tDSTOP (250 ns)
...
static void Wait_us(DWORD us)
{
	...
	{
		// Fail-safe when exceeding 64Mhz
		dwTotalNsPerCycle = 16;	
	}
