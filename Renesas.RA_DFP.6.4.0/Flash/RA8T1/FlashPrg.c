/**************************************************************************//**
 * @file     FlashPrg.c
 * @brief    Flash Programming Functions adapted for RA8T1
 * @version  V1.0.1
 * @date     06 Feb 2024
 ******************************************************************************/
/*
...
#define MCU_SCKDIVCR_ADDR               (MCU_SYSTEM_BASE+0x20)
    #define SCKDIVCR_FCLK_1DEV_MASK     (0x0FFFFFFF)
    #define SCKDIVCR_ICLK_1DEV_MASK     (0xF0FFFFFF)
		#define SCKDIVCR_FCK_MASK           (0xF0000000)
...
#define MCU_PLLCCR_ADDR                 (MCU_SYSTEM_BASE+0x28)
    ...
    #define PLLCCR_PLIDIV_3DEV          (0x0002)
		#define PLLCCR_PLIDIV_4DEV          (0x0003)
		
		#define PLLCCR_PLLMULNF_MASK       	(0x00C0)
		#define PLLCCR_PLLMULNF_0ADD_MUL  	(0x0000)
		#define PLLCCR_PLLMULNF_1_3ADD_MUL  (0x0040) 
		#define PLLCCR_PLLMULNF_2_3ADD_MUL  (0x0080) 
		#define PLLCCR_PLLMULNF_1_2ADD_MUL  (0x00C0)
...
#define MCU_OFS1_ADDR										(0x0300A200)
...
#define MCU_FLASH_BASE                 (0x40110000)
#define MCU_FCKMHZ_ADDR                (MCU_FLASH_BASE + 0xC040)
#define MCU_FACI_CMD_AREA_ADDR         (0x40100000)
...
#define MCU_FWEPROR_ADDR               (0x4001EA54)

/*
   Other parameters
*/
#define DUALSEL_ADDR 								    (0x0300A110)
#define FPCKAR_KEY              				(0x1E00)
#define FWEPROR_PE_ENA          				(0x01)
#define DATAF_MASK_ADDR         				(0xFFFFFFFF)
#define MCU_CONFAREA_CMPADDR_CODE_FLASH (0x0300A100)
#define MCU_CONFAREA_CMPADDR_DATA_FLASH (0x27030050)
#define MCU_FENTRYD_CMPADDR     				(0x27000000)
#define MCU_FENTRYC_CMPADDR     				(0x02000000)
#define CODEF_MASK_ADDR         				(0x0FFFFFFF)
...
#ifdef RA8T1_1M
	#define CODEF_32K_BLOCK_NUM     			(0x1E)
	#define DATAF_64B_BLOCK_NUM     			(0xC0)
#endif
...
#ifdef RA8T1_2M
	#define CODEF_32K_BLOCK_NUM     			(0x3D)
	#define DATAF_64B_BLOCK_NUM     			(0xC0)
#endif

#ifdef RA8T1_2M_DL
	#define CODEF_32K_BLOCK_NUM     			(0x1D)
	#define DATAF_64B_BLOCK_NUM     			(0xC0)
#endif
...
#ifdef RA8T1_CCONF //The Configuration area does not support Erase, the default number for blocks 8KB and 32KB is 0. 
	#define CODEF_32K_BLOCK_NUM     			(0x0)
	#define DATAF_64B_BLOCK_NUM     			(0x0)
#endif

#ifdef RA8T1_DCONF //The Configuration area does not support Erase, the default number for blocks 8KB and 32KB is 0. 
	#define CODEF_32K_BLOCK_NUM     			(0x0)
	#define DATAF_64B_BLOCK_NUM     			(0x0)
#endif
...
/*
   FCKMHZ Change Flag
	 0:FCLK low to high change
	 1:FCLK high to low change
*/
static udword_t dwFCKMHZChgFlg;
...
static void set_fckmhz(udword_t *udwp_clk);
...
int Init (unsigned long adr, unsigned long clk, unsigned long fnc)
{
    ...
    dwICLKValue = 240000000;
	
		// Initialize FCKMHZ Change Flag
		dwFCKMHZChgFlg = 0;
...
int BlankCheck (unsigned long adr, unsigned long sz, unsigned char pat)
{
		...
		// Detect Area
		if ((adr < MCU_FENTRYD_CMPADDR) || (adr >= MCU_CONFAREA_CMPADDR_DATA_FLASH))
		{
				// This is Code flash and Configuration Area for Code flash and Configuration Area for Data flash
				// Read target area and perform blank check
				M16(MCU_FENTRYR_ADDR) = FENTRYR_READ_MODE; // Transition to read mode
...
int EraseChip (void)
{
    ...
    uword_t uw_block_total_num;
		uword_t uw_block_add_num = 0;	// Number of additional blocks in dual mode
    uword_t i;
	
#if (defined RA8T1_CCONF) || (defined RA8T1_DCONF) 
	  /** Configuration area not support EraseChip */
    return (0); // OK
#endif
	
#if	(defined	CODE_1M_dual) || (defined	CODE_2M_dual)
		uw_block_add_num = CODEF_8K_BLOCK_NUM + CODEF_32K_BLOCK_NUM;
#endif	

    uw_block_total_num = CODEF_8K_BLOCK_NUM + CODEF_32K_BLOCK_NUM + DATAF_64B_BLOCK_NUM + uw_block_add_num;

    /** Repeat the process for the number of blocks of Code Flash and Data Flash */
    for(i = 0; i < uw_block_total_num; i++)
    {
				if (uw_block_add_num == 0) // linear mode
				{
            ...
            if (i == 0)
		        ...
		        {
                udw_addr += 0x2000; //number address for Block 8KB
            } else if (CODEF_8K_BLOCK_NUM < i && i < (CODEF_8K_BLOCK_NUM + CODEF_32K_BLOCK_NUM))
            {
                udw_addr += 0x8000; //number address for Block 32KB
            ...
            {
                udw_addr += 0x40; //number address for Block 64B
            }
			  } else // dual mode
				{
		
		        /** Calculate the start address of a block from the size and number of blocks */
            if (i == 0) // The beginning of the lower side code flash
    				{
                pe_mode_entry(MCU_FENTRYC_CMPADDR, &udw_addr); // Transition to P/E mode
	    	    } else if (0 < i && i < CODEF_8K_BLOCK_NUM) // The lower side code flash area (8KB Block)
    		    {
                udw_addr += 0x2000; //number address for Block 8KB
            } else if (CODEF_8K_BLOCK_NUM <= i && i < (CODEF_8K_BLOCK_NUM + CODEF_32K_BLOCK_NUM)) // The lower side code flash area (32KB Block)
            {
                udw_addr += 0x8000; //number address for Block 32KB
            }  else if (i == CODEF_8K_BLOCK_NUM + CODEF_32K_BLOCK_NUM) // The beginning of the apper side code flash
    		    {
                udw_addr = MCU_FENTRYC_CMPADDR + 0x200000;
            } else if ((CODEF_8K_BLOCK_NUM + CODEF_32K_BLOCK_NUM) < i 
						&& i < (CODEF_8K_BLOCK_NUM + CODEF_32K_BLOCK_NUM + CODEF_8K_BLOCK_NUM)) // the upper side Code flash area (8KB Block)
            {
                udw_addr += 0x2000; //number address for Block 8KB
						} else if ((CODEF_8K_BLOCK_NUM + CODEF_32K_BLOCK_NUM + CODEF_8K_BLOCK_NUM) <= i 
						&& i < (CODEF_8K_BLOCK_NUM + CODEF_32K_BLOCK_NUM + uw_block_add_num)) // The upper side Code flash area (32KB Block)
            {
                udw_addr += 0x8000; //number address for Block 32KB
            }else if (i == CODEF_8K_BLOCK_NUM + CODEF_32K_BLOCK_NUM + uw_block_add_num) // First block of Data Flash
            {
                pe_mode_entry(MCU_FENTRYD_CMPADDR, &udw_addr); // Transition to P/E mode
            } else 
            {
				...
				M32(MCU_FSADDR_ADDR) = udw_addr; // Set start address of target block
				M8(MCU_FACI_CMD_AREA_ADDR) = FACI_ERASE1_CMD; // Execution of erase command (0x20).
				M8(MCU_FACI_CMD_AREA_ADDR) = FACI_END_CMD; // Execution of termination command (0xD0).
...
int EraseSector (unsigned long adr)
{
    ...
    if (adr < MCU_CONFAREA_CMPADDR_CODE_FLASH)
    {
        ; // Execute EraseSector command if Code Flash
    } else if (adr < MCU_FENTRYD_CMPADDR)
    {
        /** Configuration area for Code flash not support EraseSector */
        return (0); // OK
    } else if (adr < MCU_CONFAREA_CMPADDR_DATA_FLASH)
    {
        ; // Execute EraseSector command if Data Flash
    } else
		{
				/** Configuration area for Data flash not support EraseSector */
				return (0); // OK
		}
...
int ProgramPage (unsigned long adr, unsigned long sz, unsigned char *buf)
{
    ...
    while (sz)
    {
				...
				{
						uby_area_flg = 2; // Configuration area for Code flash
						if (sz < 16)
						{
								for (i = sz; i < 16; i++)
								{
				...
				{
						uby_area_flg = 2; // Configuration area for Data flash
							if (sz < 16)
							{
									for (i = sz; i < 16; i++)
									{
...
static udword_t check_clock_and_mode(udword_t *udwp_clk)
{
    ...
    udw_sckdivcr = (udw_sckdivcr & SCKDIVCR_FCK_MASK) >> 28;	
		...
		{ 
			udw_sckdivcr_val = 3 * (1 << (udw_sckdivcr - 8)); // FCLK division value
    ...
    if (((*udwp_clk) < 4000000) || (60000000 < (*udwp_clk)))
    {
		...
		set_fckmhz(udwp_clk); // FCKMHZ register setting
...
static udword_t change_clock_and_mode(udword_t *udwp_clk)
{
    ...
    if ((*udwp_clk) < 4000000) // FCLK low to high change
    {
        dwFCKMHZChgFlg = 0;
    }
		else if (60000000 < (*udwp_clk)) // FCLK high to low change
		{
				dwFCKMHZChgFlg = 1;
		}
		else
		{
				; // This process does not pass because there is no change in FCLK
		}
		
		/** Set the FCKMHZ register here to raise the current FCLK */
		if (dwFCKMHZChgFlg == 0)
		{
			set_fckmhz(udwp_clk); // FCKMHZ register setting
		}
    ...
    {/** If OFS1 is not in the initial state, set the clock source to HOCO */
        ...
        switch (s_udwClockUserVal[OFS1] & OFS1_HOCOFRQ0_MASK)
        {
            ...
            break;
                ...
                *udwp_clk = 32000000; // FCLK frequency is 32MHz
            break;
            case OFS1_HOCOFRQ0_48MHZ:
                *udwp_clk = 48000000; // FCLK frequency is 48MHz
            break;
            default:
                return (1); // Failed
        }
	  ...
	  /** Set the FCKMHZ register here because we lowered the current FCLK */
		if (dwFCKMHZChgFlg == 1)
		{
			set_fckmhz(udwp_clk); // FCKMHZ register setting
		}
...
static ubyte_t saveModeAndClock(void)
{
  ...
  s_udwClockUserVal[SCKDIVCR] = M32(MCU_SCKDIVCR_ADDR);
	...
	s_udwClockUserVal[FCKMHZ] = (udword_t)M8(MCU_FCKMHZ_ADDR);
...
static udword_t restore_clock_and_mode(void)
{
     ...
     * because it is a type conversion from unsigned to unsigned */
		...
		if (dwFCKMHZChgFlg == 1)
		{
			M8(MCU_FCKMHZ_ADDR) = (ubyte_t)s_udwClockUserVal[FCKMHZ];
		}
...
/**************************************************************************//**
* @details   FCKMHZ register setting
* @param[in] udwp_clk:  Clock Frequency (Hz)
* @retval    None
******************************************************************************/
static void set_fckmhz(udword_t *udwp_clk)
{
		...
		ubyte_t ub_mhz_fckmhz;
	
		ub_mhz_fckmhz = (*udwp_clk) / 1000000; // Conver to MHz unit

    if ((*udwp_clk) % 1000000)
    {
        ub_mhz_fckmhz += 1; // Round up the first decimal place in MHz
    }
		M8(MCU_FCKMHZ_ADDR) = ub_mhz_fckmhz;
...
static void Wait_us(udword_t us)
{
	...
	if (dwTotalNsPerCycle < 10)
	{
		// Fail-safe when exceeding 100Mhz
		dwTotalNsPerCycle = 10;	
	}
