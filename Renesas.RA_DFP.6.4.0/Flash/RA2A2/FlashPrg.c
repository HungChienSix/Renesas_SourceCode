/**************************************************************************//**
 * @file     FlashPrg.c
 * @brief    Flash Programming Functions adapted for RA2A2
 * @version  V1.0.2
 * @date     28 March 2024
 ******************************************************************************/
/*
 ...
 *         : 05.02.2024 1.0.1    Increase Programming Page Size 
 *         : 28.03.2024 1.0.2    Modify Programming Page Size (Code/Data - 2KB/1KB)
 *********************************************************************************************************************/
...
#define MCU_FEXCR_ADDR									(MCU_FLASH_BASE + 0x1DC)
		...
		#define FEXCR_CMD_FLASH_CONTROL_FLAG		(0x01)
...
#define MCU_CODEF_MACRO_SIZE						(0x00040000)

#define MCU_FCF_ADDR										(CONFIG_START + 0x08)
...
#ifdef RA2A2_512K
	DWORD dwEraseChipStartAdr = MCU_CODEF_BASEADDR;
	// Clear Code flash
	while (dwEraseChipStartAdr < MCU_CODEF_BASEADDR + MCU_CODEF_SIZE)
	{
		// The size that can be erased at once is 256KB
		byteIsFailed |= EraseChipArea(dwEraseChipStartAdr, MCU_CODEF_MACRO_SIZE);
		if (byteIsFailed)
		{
			return (byteIsFailed); // Return status
		}
		dwEraseChipStartAdr += MCU_CODEF_MACRO_SIZE;
	}
#endif
#ifdef RA2A2_DATA
	// Clear Data flash
...
int ProgramPage (unsigned long adr, unsigned long sz, unsigned char *buf)
{
	...
	} else
	{
		...
		{
		case MCU_FCF_ADDR:
			byteExCmd = FEXCR_CMD_FLASH_CONTROL_FLAG;
		case MCU_AWS_ADDR:
...
static DWORD checkClockAndMode(DWORD *clk)
{
  ...
  switch (wClockUserVal[SCKSCR])
  {
    ...
    case SCKSCR_CKSEL_LOCO: // Failed because LOCO frequency is below 1MHz
				dwordClk = 32768;
				break; 
    case SCKSCR_CKSEL_SOSC: // Failed because SOSC frequency is below 1MHz
				dwordClk = 32768;
				break;			
    default:
        return (RES_ERROR); // Failed
